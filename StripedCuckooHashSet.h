...
class StripedCuckooHashSet : public PhasedCuckooHashSet<T> {
    private:
        ...
        void acquire(T x){
            mutex* lock0 = locks[0][this->hash0(x)];
            mutex* lock1 = locks[1][this->hash1(x)];
        ...
        void resizeLocks(){
            ...
            for(int i = 0; i < 2; ++i){
                newlocks[i] = (mutex*)malloc(sizeof(mutex) * this->capacity * 2);
    ...
    public:
        StripedCuckooHashSet() : PhasedCuckooHashSet<T>(){
            ...
            for(int i = 0; i < 2; ++i){
                locks[i] = (mutex**)malloc(sizeof(void*) * N);
                for(int j = 0; j < N; ++j)
                    locks[i][j] = new mutex();
            }
